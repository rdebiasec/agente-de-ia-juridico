...
def create_slack_app() -> AsyncApp | None:
    ...
    async def on_mention(event, say, client):
        ...
        await say(result["text"], thread_ts=event.get("ts"))
    ...
    async def on_message(message, say):
        ...
        result = await handle_message(
            InboundMessage(channel="slack", user_id=user, text=text)
        )
        await say(result["text"], thread_ts=message.get("thread_ts") or message.get("ts"))
