...
|---|---|---|---|
| A1 | Health prod OK + postgres | `curl -s https://agente-de-ia-juridico.onrender.com/health` | [ ] |
| A2 | Environment=production | mismo JSON (`environment`) | [ ] |
| A3 | Mapa superficies | Chat `/`, `/cliente`, `/auditoria`, `/legal/*`, Slack HITL | [ ] |
| A4 | WhatsApp no activo | `ESTADO_PROYECTO.md` + ausencia gateway WA | [ ] |
...
| B5 | IP allowlist (si aplica) | `IP_ALLOWLIST_ENABLED` / middleware | [ ] |
| B6 | Sin secretos en git | `.gitignore` + no `.env` tracked; solo `.env.example` | [ ] |
...
| C3 | Consent `/cliente` | `triple_chat_api.py` `consent_1581` | [ ] |
| C4 | ARCO erase | `POST /api/compliance/arco-erase` + runbook | [ ] |
| C5 | Páginas legales | `/legal/terminos`, privacidad, tratamiento casos | [ ] |
| C6 | Retención programada | `purge_retention.py` / compliance retention | [ ] |
| C7 | DPA encargados | `ESTADO_DPA_Y_ARCO.md` — firmados vs plantilla | [ ] |
| C8 | Flags menores/sensibles | expediente `involucra_menor` / `datos_sensibles` | [ ] |
| C9 | Cifrado en reposo | Fernet / `DATA_AT_REST_KEY` o derivado de `SESSION_SECRET` | [ ] |

### D. Seguridad IA + HITL (60 min)

| # | Check | Cómo / dónde | Resultado |
|---|---|---|---|
| D1 | I/O/T por agente en disco | `config/guardrails/agents/{id}/{input,output,tools}.md` | [ ] |
| D2 | Cableado SDK | `sdk_guardrails.py` + `orchestrator.py` | [ ] |
| D3 | Tutela/otro equipo → OOS | `triage.is_other_team_scope_request` + evals | [ ] |
| D4 | Memoriales requieren plan HITL | `plan_templates` / runner / drafts | [ ] |
| D5 | Disclaimer salida | apply_output_guardrails / g8=`aviso_borrador` | [ ] |
| D6 | No inventar + pendiente | prompts + soft-flag invention | [ ] |
| D7 | Tools chat sin redactor libre | superficie tools del POC | [ ] |
...
| # | Check | Cómo / dónde | Resultado |
|---|---|---|---|
| E1 | Postgres prod healthy | Render DB + health `persistencia=postgres` | [ ] |
| E2 | Alembic al día | `alembic_version` / migrate on boot | [ ] |
| E3 | Backup offsite | `.github/workflows/backup-postgres.yml` último success | [ ] |
| E4 | Restore documentado | `scripts/dr/*` + runbook | [ ] |
| E5 | Fecha último restore drill | ops note | [ ] |
| E6 | Drift config archivo↔DB | `sync_config_files.py --check` (prod secret) | [ ] |
| E7 | Sin huérfanos tutela en `config_active` | query keys `%tutela%` = 0 | [ ] |
...
| F2 | pytest guardrails/HITL/routing | `tests/test_guardrails_iot_*`, `test_firma`, evals | [ ] |
| F3 | Evals OOS / memorial | `config/evals/agent_eval_cases.json` | [ ] |
| F4 | Sync config en push | `sync-config.yml` | [ ] |
...
| # | Check | Cómo / dónde | Resultado |
|---|---|---|---|
| G1 | UI indica borrador / revisión humana | chat + firma | [ ] |
| G2 | `/cliente` no sobrepromete | copy + gaps formacion | [ ] |
| G3 | Entregables comerciales alineados | `docs/entregables/*` sin “tutela completa / abogado IA” | [ ] |
| G4 | Prompts dicen “propone; abogado aprueba” | `sistema.md` / coordinador | [ ] |

### H. FinOps + observabilidad + terceros (30–45 min)

| # | Check | Cómo / dónde | Resultado |
|---|---|---|---|
| H1 | Modelo high-risk ≠ default chat | `OPENAI_MODEL` vs `OPENAI_MODEL_HIGH_RISK` | [ ] |
| H2 | Caps tokens/turns | `AGENT_MAX_*` en config | [ ] |
