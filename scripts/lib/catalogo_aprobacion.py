...
AGENTS = [
    ...
    },
    {
        ...
        "no_reemplaza": "La intervencion oral de la abogada en estrados.",
        "prompt_simple": [
    ...
    },
    {
        ...
        "necesidad": "Refuerza responsabilidad profesional de la abogada y soporte de auditoria interna.",
        "no_reemplaza": "La aprobacion final de la abogada.",
        "prompt_simple": [
...
def parse_lista_steps() -> dict[str, dict]:
    ...
    while i < len(blocks) - 1:
        ...
        steps = re.findall(r"^\s*\d+\.\s+(.+)$", content, re.M)
...
def build_skill_steps(raw_steps: list[str]) -> list[dict]:
    return [{"num": i, "text": t.strip()} for i, t in enumerate(raw_steps, start=1) if t.strip()]
